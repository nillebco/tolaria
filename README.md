...
table:
  ...
  density: compact
  summaries:
    "computed:quantity": sum
    "computed:amount": sum
    "computed:crateSize":
      type: formula
      value: 'if(sum("computed:quantity") <= 70, 5, 5 + (sum("computed:quantity") - 70) * 2)'
...
`table.computedColumns` supports simple aliases such as `quantity: Quantity`, plus
...
| Number functions | `max(quantity, 70)`, `min(quantity, 70)` |
...
Summaries also accept object form, either `{ type: sum }` or
`{ type: formula, value: '...' }`. Summary formulas can use
`sum("column:id")` to total a visible table column after filters, for example
`sum("property:quantity")` or `sum("computed:amount")`.
