...
Command-layer path access is fenced to the active vault before file operations reach the vault backend. `src-tauri/src/commands/vault/boundary.rs` canonicalizes the configured/requested vault root, rejects `..` escapes and absolute paths outside that root, and validates writable targets through the nearest existing ancestor so note reads, saves, deletes, view-file edits, folder mutations, and image attachment writes cannot step outside the active vault. Image attachment commands use Obsidian's `.obsidian/app.json` `attachmentFolderPath` when present, otherwise preserve an existing `attachments` / `Attachments` folder casing before falling back to `attachments`, then refresh the runtime asset scope after saving.

Renderer image normalization keeps `![[...]]` embeds portable before BlockNote parses them. Explicit embed paths are honored as written. Bare image filenames are resolved through the current vault file index when the filename is unique across the vault; ambiguous or missing filenames fall back to the detected attachments folder casing, or `Attachments/<filename>` when no attachments folder is indexed yet.
...
- The block-handle side menu keeps only actions that survive Tolaria's markdown round-trip. Delete and table-header toggles remain available; BlockNote's `Colors` submenu is removed because block colors are not part of Tolaria's supported markdown surface. Block reordering uses a Tolaria-owned pointer gesture instead of BlockNote's HTML5 drag/drop path, because Tauri's WKWebView can emit `dragstart`/`dragend` without a matching `drop`. The pointer path resolves the target block by hit-testing first and by nearest vertical block position as a fallback, so dragging from the left gutter still commits the reorder.
